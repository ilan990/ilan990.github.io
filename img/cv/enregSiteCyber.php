<?php 
	...
	MP: Preferer les POST que les GET
	$nom = $_GET["nom"];
	$log = $_GET["log"];
	$age = $_GET["age"];
	$pass = $_GET["mdp"];
	...
	try 
	{
		...
		if($_GET["valenreg"]) {	
			...
			FAILLE: Injection SQL. Il suffit de connaître le login (exemple:TOTO) et de mettre dans le formulaire, au champ
			login ( TOTO;--). Ca coupera la requete en recherchant juste le nom.
